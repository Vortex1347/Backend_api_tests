...
from conftest import assert_success, confirm_operation, create_metadata, make_grpc_request
from data import ADMIN_API_URL, ADMIN_SESSION_KEY
from database_collector import DatabaseConfig, DataCollector

CASE_FILE = Path(__file__).resolve().parent / 'testdata' / 'bank_client_transfer_cases.json'
SESSION_RETRY_LIMIT = 10
POLL_INTERVAL_SECONDS = 2
BALANCE_SYNC_TIMEOUT_SECONDS = 90
...
def _load_cases() -> list[dict[str, Any]]:
    ...
    for case in cases:
        ...
        if not name:
            raise ValueError('Each live matrix case must define a non-empty name')
        if name in seen_names:
            raise ValueError(f'Duplicate live matrix case name detected: {name}')
        if not route_key:
            raise ValueError(f'Case {name} is missing required route_key')
        if case.get('expected_error'):
            raise ValueError(
                f'Case {name} still contains expected_error. '
                'Active live matrix must contain only pass-backed routes.'
            )
        seen_names.add(name)
...
def _build_case_metadata(case: dict[str, Any], session_key: str):
    return create_metadata(
        ...
        device_type=case.get('device_type', 'ios'),
        user_agent=case.get('user_agent', '12; iPhone12MaxProDan'),
    )
...
def _build_admin_headers() -> dict[str, str]:
    return {
        ...
        'user-agent-c': 'chrome',
        'accept': '*/*',
        'referer': f'{ADMIN_API_URL}/admin-ui/cards',
        'origin': ADMIN_API_URL,
    }


def _count_sync_cards(payload: dict[str, Any]) -> int:
...
def _sync_account_view(account: dict[str, Any]) -> dict[str, Any] | None:
    ...
    response = requests.post(
        f'{ADMIN_API_URL}/adminApi/others/cards',
    ...
    print(
        ...
        f"sync_cards_count={_count_sync_cards(payload)}, payload={json.dumps(payload, ensure_ascii=False, default=str)}"
    )
    return payload
...
def _create_request_with_session_retry(
    ...
    *,
    ...
    for offset in range(SESSION_RETRY_LIMIT):
...
def _prepare_case_accounts(
    ...
    sender_sync_payload = _sync_account_view(sender_account)
    recipient_sync_payload = _sync_account_view(recipient_account) if recipient_account else None
    ...
    requested_debit_amount = _parse_decimal(case['request'].get('amountDebit') or case['request'].get('amountCredit'))
...
def _wait_for_expected_transaction(
    ...
    *,
    ...
    timeout_seconds = int(verification.get('transaction_timeout_seconds', TRANSACTION_TIMEOUT_SECONDS))
...
def _expected_sender_delta(case: dict[str, Any], transaction: dict[str, Any]) -> Decimal:
    ...
    if code in {'MAKE_OTHER_BANK_TRANSFER', 'MAKE_SWIFT_TRANSFER', 'MAKE_GENERIC_PAYMENT_V2'}:
        return amount_debit_total or amount_debit or Decimal('0')
    return amount_debit or Decimal('0')
...
def _verify_success_transaction(
    ...
    *,
    ...
    if code == 'MAKE_GENERIC_PAYMENT_V2':
        assert transaction['service_provider_id'] == request['serviceProviderId']
        assert transaction['prop_value'] == request['propValue']
        return

    pytest.fail(f"{case['name']}: unsupported verification for operation code={code}")


def _handle_expected_create_error(
    collector: DataCollector,
    *,
    case: dict[str, Any],
    operation_id: str,
    ...
    if getattr(create_response, 'success', False):
        print(
            f"[expected-error] {case['name']} succeeded even though "
            f"expected_error={expected_error['code']} is configured; continuing with success path"
        )
        return False

    actual_error_code = _get_transaction_error_code(create_response)
    actual_error_data = _get_transaction_error_data(create_response)
    assert actual_error_code == expected_error['code'], (
        f"{case['name']}: expected error_code={expected_error['code']}, got {actual_error_code}. "
        f'error_data={actual_error_data}'
    )

    transaction = collector.get_transaction_by_operation_id(operation_id)
    assert transaction is None, (
        f"{case['name']}: expected no transaction row for known create failure, got "
        f"{json.dumps(transaction, ensure_ascii=False, default=str, indent=2)}"
    )

    pytest.xfail(expected_error['reason'])


@pytest.mark.parametrize('case', TEST_CASES, ids=[case['name'] for case in TEST_CASES])
def test_bank_client_transfer_live(case: dict[str, Any]):
    ...
    if _handle_expected_create_error(
        collector,
        case=case,
        operation_id=operation_id,
